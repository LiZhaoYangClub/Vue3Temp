...
import { HttpInstance } from './axios-instance'
...
const transform: AxiosTransform = {
  ...
  beforeRequestHook: (config, options) => {
    ...
    if (config.method?.toUpperCase() === RequestEnum.GET) {
      ...
      if (!isString(params)) {
        ...
        if (
          config.headers &&
          Reflect.has(config.headers, 'Content-Type') &&
          config.headers.get('Content-Type') === ContentTypeEnum.FORM_DATA
        ) {
  ...
  handlerAbnormalCode: (code: string, msg: string, result?: any): void => {
    console.log(code, msg, result)
  }
}

const createAxios = (opt?: Partial<CustomAxiosOptions>) =>
  new HttpInstance(
    deepMerge(
      {
